...
def computer_width_height(detected_corner):
    ratio = 210 / 297
    len_1 = np.sqrt((detected_corner[0, 0] - detected_corner[1, 0]) ** 2 + (detected_corner[0, 1] - detected_corner[1, 1]) ** 2)
    len_2 = np.sqrt((detected_corner[1, 0] - detected_corner[2, 0]) ** 2 + (detected_corner[1, 1] - detected_corner[2, 1]) ** 2)
    len_3 = np.sqrt((detected_corner[2, 0] - detected_corner[3, 0]) ** 2 + (detected_corner[2, 1] - detected_corner[3, 1]) ** 2)
    len_4 = np.sqrt((detected_corner[3, 0] - detected_corner[1, 0]) ** 2 + (detected_corner[3, 1] - detected_corner[1, 1]) ** 2)

    len_max = max([len_1, len_2, len_3, len_4])
    if len_max==len_1 or len_max==len_3:
        x_max = np.uint16(len_max)
        y_max = np.uint16(x_max * ratio)
    else:
